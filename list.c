...
node_q3 *new_node_q3(char *val, char *val2, char *val3,char *val4)
{
    ...
    node_q3 *temp = (node_q3 *)emalloc(sizeof(node_q3));

    temp->portname = strdup(val);
    temp->portshort = strdup(val2);
    temp->portcity = strdup(val3);
    temp->portcountry = strdup(val4);
...
node_q3 *add_end_q3(node_q3 *list, node_q3 *new)
{
    node_q3 *curr;
...
node_t *add_inorder2(node_t *list, node_t *new)
{
    ...
    for (curr = list; curr != NULL; curr = curr->next)
    {
        if (new->duplicate < curr->duplicate)
        {
            prev = curr;
        }
        else if(new->duplicate == curr->duplicate){
            if (strcmp(new->word , curr->word)>0){
                prev = curr;
            }
        }
...
node_q3 *peek_front_q3(node_q3 *list)
{
    return list;
}
...
node_q3 *remove_front_q3(node_q3 *list)
{
    if (list == NULL)
    {
        return NULL;
